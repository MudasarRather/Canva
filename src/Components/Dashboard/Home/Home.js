...
const Home = (props) => {
  ...
  useEffect(() => {
    Analytics();
    // axios
    //   .post(conf.endPoint + "/api/auth/check", {}, { withCredentials: true })
    //   .then((resp) => {
    //     debugger;
    ...
    //   });
    setUser({
      _id: "63669ec477f877397aceacd2",
      email: "rt@gmail.com",
      firstname: "Welcome",
      lastname: "Back",
      plan: "Premium",
      subcrtiptionStarts: null,
      subcrtiptionEnds: "2023-01-01T00:00:00.000Z",
      role: "admin",
      __v: 0,
    });
    setIsLoggedIn(true);
  ...
  const logout = () => {
    axios
      .post(conf.endPoint + "/api/auth/logout", {}, { withCredentials: true })
      .then((resp) => {
        console.log(resp);
  ...
  return isLoggedIn == true ? (
    <div className="dashboard flex-col">
            ...
            {meta.subscription == true && (
              <li>
                <Link to={"/upgrade"} href="#">
                  {" "}
                  <div className="small-highlight">pro</div> Upgrade
                </Link>
              </li>
            )}
      ...
      <div className="dashboard__body">
        ...
        <Switch>
          <Route
            path={"/members/editor/:id"}
            render={(props) => <Editor monthly={monthly} user={user} />}
          />
