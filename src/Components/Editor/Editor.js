...
const Editor = (props) => {
  ...
  const [selectedElementArrayIndex, setSelectedElementArrayIndex] =
    useState(null);
  ...
  useEffect(() => {
    ...
    try {
      axios
        .get(`${conf.endPoint}players/6381019a3bc74f4118cf8055`)
        .then((resp) => {
          debugger;
          console.log(resp.data);
          if (resp.data.result) {
            // var dim = resp.data.size.split("x");
            var dim = [resp.data.data.Width, resp.data.data.Height];

            // var dim = "1000x700".split("x");
            ...
            // } else {
            setselectedTemplatePlan("Free");
            // }
            loadFromJson(JSON.parse(resp.data.data.Source));
            switch ("Custom") {
              case "Facebook Post":
                setStageDimensions({
                ...
                setxSnapPoints([
                  ...[0, sizes.fbPost.width / 2, sizes.fbPost.width],
                ]);
                break;
              case "Instagram Post":
                setStageDimensions({
                ...
                setxSnapPoints([
                  ...[0, sizes.inPost.width / 2, sizes.inPost.width],
                ]);
                break;
              case "Twitter Post":
                setStageDimensions({
                ...
                setxSnapPoints([
                  ...[0, sizes.twPost.width / 2, sizes.twPost.width],
                ]);
                break;
              case "Youtube Thumbnail":
                ...
                setxSnapPoints([
                  ...[
                    0,
                    sizes.youtubeThumbnail.width / 2,
                    sizes.youtubeThumbnail.width,
                  ],
                ]);
                break;
              case "Custom":
  ...
  const handleLockClick = () => {
    ...
    if (tempArray.length > 0) {
      tempArray[selectedElementArrayIndex].isLocked =
        !tempArray[selectedElementArrayIndex].isLocked;
      setElements([...tempArray]);
    }
  };
      ...
      <div className="editor__workarea">
        ...
        >
          ...
          <Lock
            ...
            selectedLockedStatus={selectedLockedStatus}
          />
        ...
        <div
          style={{
            transform:
              zoom >= 100
                ? "scale(" +
                  zoom.toString().substring(0, 1) +
                  "." +
                  zoom.toString().substring(1, 2) +
                  ")"
            ...
            <Layer
              ...
              onDragMove={(event) => {
                ...
                for (let index = 0; index < xSnapPoints.length; index++) {
                  ...
                  if (
                    event.target.x() + event.target.getClientRect().width >=
                      xSnapPoints[index] - 3 &&
                    event.target.x() + event.target.getClientRect().width <=
                      xSnapPoints[index] + 3
                  ) {
                    event.target.x(
                      xSnapPoints[index] - event.target.getClientRect().width
                    );
                    var newXGuidelines = XGuideLines;
                    newXGuidelines.push([
                      xSnapPoints[index],
                  ...
                  if (
                    event.target.x() + event.target.getClientRect().width / 2 >=
                      xSnapPoints[index] - 3 &&
                    event.target.x() + event.target.getClientRect().width / 2 <=
                      xSnapPoints[index] + 3
                  ) {
                    event.target.x(
                      xSnapPoints[index] -
                        event.target.getClientRect().width / 2
                    );
                    var newXGuidelines = XGuideLines;
                    newXGuidelines.push([
                      xSnapPoints[index],
                ...
                for (let index = 0; index < elements.length; index++) {
                  if (elements[index].name !== event.target.name()) {
                    ...
                    if (
                      ...
                      event.target.x(
                        elements[index].x - event.target.getClientRect().width
                      );
                      var newXGuidelines = XGuideLines;
                      newXGuidelines.push([
                        elements[index].x,
